...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        evaluated_expr = scheme_eval(first, env)
        if isinstance(evaluated_expr, MacroProcedure):
            return scheme_eval(evaluated_expr.apply_macro(rest, env), env)
        if isinstance(evaluated_expr, Procedure):
            args = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(evaluated_expr, args, env)
        else:
            check_procedure(evaluated_expr)
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # Two cases:
            # 1. Global frame
                # If the frame is the global frame, just look it up currently and if it doesn't exist, raise an error.
        ...
        if self.parent == None:
            ...
            if symbol in self.bindings:
                return self.bindings.get(symbol)
            elif symbol not in self.bindings:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil:
            ...
            vals = vals.second
            if formals == nil and vals != nil or formals != nil and vals == nil:
                raise SchemeError
        return child_frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        expression_val = scheme_eval(expressions.second.first, env)
        env.define(target, expression_val)
        return target
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0 or expressions == nil:
        return True
...
def make_let_frame(bindings, env):
    ...
    bindings2 = bindings
    while bindings2 != nil:
        check_form(bindings2.first, 2, 2)
        bindings2 = bindings2.second
    list_to_check = bindings.map(lambda x: x.first)
    list_of_values = bindings.map(lambda x: scheme_eval(x.second.first, env))
    check_formals(list_to_check)
    return env.make_child_frame(list_to_check, list_of_values)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return target
         ...
         env.define(target.first, MacroProcedure(expressions.first.second, expressions.second, env))
         return target.first
    else:
         bad_target = target.first if isinstance(target, Pair) else target
         raise SchemeError('non-symbol: {0}'.format(bad_target))
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        dynamicallyscoped_frame = env.make_child_frame(self.formals, args)
        return dynamicallyscoped_frame
    # END PROBLEM 16
