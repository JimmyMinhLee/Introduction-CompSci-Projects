...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    score_you_earn = 0
    if num_rolls != 0:
        score_you_earn = roll_dice(num_rolls, dice)
    else:
        score_you_earn = free_bacon(opponent_score)
    return score_you_earn
...
def is_swap(player_score, opponent_score):
    ...
    a = player_score % 10
    b = (player_score // 10) % 10
    c = abs(a - b)
    d = opponent_score % 10
    e = (opponent_score // 10) % 10
    f = abs(d - e)
    if c == f:
        return True
    else:
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        new_high, new_score = previous_high, previous_score
        if who == 0:
            change_in_score = score0 - new_score
            if change_in_score > new_high:
                new_high = change_in_score
                print(new_high, "point(s)! That's the biggest gain yet for Player 0")
            new_score = score0
        else:
            change_in_score = score1 - new_score
            if change_in_score > new_high:
                new_high = change_in_score
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    #You should use make_averaged and roll_dice
    x, b, max_score, average = 1, 1, 0, make_averaged(roll_dice, num_samples)
    while x <= 10:
        average_score = average(x, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    new_score = free_bacon(opponent_score) + score
